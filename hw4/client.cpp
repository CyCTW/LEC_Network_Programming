...
int main(int argc, char const **argv) {
	...
	cout << buf_str;
	int postId = 0;
	int key_idx = sockfd*1000;
	
	int offset_mailid = 0;
	
	bool online = false;
	string username = "";
	string groupid = string(argv[1]);
	...
	while(true) {
		...
		if( argu[0] == "register") {
			...
			if (buf_str.substr(0, 4) == "Sent") {
				...
				stringstream ssm(tpp);
				string bucket_name, post_name;
				ssm >> bucket_name >> post_name;
				// write file first
				ofstream f;
			...
			// cout << "client: retr-mail:\n" << buf_str << '\n';
			if (buf_str.substr(2,1) == "S") {
				char c_buf[256];
				memset(c_buf, 0, sizeof(c_buf));
				send(sockfd, "#", 1, 0);
				recv(sockfd, c_buf, 255, 0);
				string ttp = string(c_buf);

				stringstream ss(ttp);
				string bucket_name, mail_name;
				ss >> bucket_name >> mail_name;
				// string mail_name = "NP_HW3_MAIL_" + mail_id;
			...
			if(buf_str.substr(0, 1) == "M") {
				...
				// delete content of this post to bucket
				delete_awsObject(bucket_name, mail_name);
			}
			
		}
		else if (argu[0] == "subscribe") {
			if (!online)
				buf_str += "Please login first.\n";
			else
				buf_str += subscribe_(argu);
			buf_str += "% ";

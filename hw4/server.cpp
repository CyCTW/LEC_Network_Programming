...
int main(int argc, char *argv[]) {
	...
	while(!close_server) {
		...
		for(int i=init_fd; i<=fdmax; i++) {
			if ( i != sockfd && i < server_fd) {
				continue;
			}
					...
					if (recv_st <= 0) {
						...
						// homework2 add new function
							...
							if (ret[0] == 'C') {
								...
								// push massage to kafka
								string board_name = argu[1];
								string message = board_name + "#" + title + "#" + user_name; //title + Author
								push_message(board_name,  message/*topic name, message*/);
								push_message(user_name,  message/*topic name, message*/);
							...
							pair<string, string> res = mail_to(i, argu);
							ret += res.first;
							ret += "% ";
							send(i, ret.c_str(), ret.size(), 0);
							char c_buf[256];
							...
							if (ret.substr(0, 4) == "Sent") {
								...
								cout << "bucket_name: " << bucket_name << '\n';
								cout << "object_name: " << res.second << '\n';
								string msg = bucket_name + ' ' + res.second;
								send(i, msg.c_str(), msg.size(), 0);
							}
						}
							...
							pair<string, string> res = retr_mail(i, argu);
							ret += res.first;
							// ret += "% ";
							send(i, ret.c_str(), ret.size(), 0);
							char c_buf[256];
							memset(c_buf, 0, sizeof(c_buf));
							cout << "ret in retr-mail:\n" << ret << '\n';
							if (ret.substr(2, 1) == "S") {
								recv(i, c_buf, 255, 0);
								// give bucket_name of specific user
								string user_name = online[i];
							...
							pair<string, string> res = delete_mail(i, argu);
							ret += res.first;
							ret += "% ";
							send(i, ret.c_str(), ret.size(), 0);
							char c_buf[256];
						...
						else if (argu[0] == "subscribe") {
							string emp = "#";
							send(i, emp.c_str(), emp.size(), 0);
						}
						else if (argu[0] == "unsubscribe") {
							string emp = "#";
							send(i, emp.c_str(), emp.size(), 0);
						}
						else if (argu[0] == "list-sub") {
							string emp = "#";
							send(i, emp.c_str(), emp.size(), 0);
						}
						else {

...
string create_post(int sockfd, vector<string> &argu, string &title) {
	string ret;
...
string list_post(int sockfd, vector<string> &argu) {
	...
	string list_key_posts = "SELECT NAME FROM BOARDS WHERE NAME='" + board_name + "';";
	sqlite3_exec(db, list_key_posts.c_str(), DB_check_enter, (void*)&enter, 0);
	if (enter == false) {
